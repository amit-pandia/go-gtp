...
// Conn represents a gtpprime-C connection.
//
...
type Conn struct {
	...
	*msgHandlerMap
	...
	// times the gtpprime-C endpoint is restarted.
...
// ListenAndServe creates a new gtpprime-C Conn and start serving background.
...
// Listen creates a new gtpprime-C Conn
...
// Serve starts serving gtpprime connection.
...
// The key of the map is message type of the gtpprime-C message. You can use MsgTypeFooBar
...
func (c *Conn) ParseCreateSession(raddr net.Addr, ies ...*ie.IE) (*Session, error) {
	...
	for _, i := range ies {
		...
		switch i.PacketTransferCommand {
